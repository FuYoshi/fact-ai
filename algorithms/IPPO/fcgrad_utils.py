...
def beta_weighting(g_ind: PyTree, g_col: PyTree, beta: float = 0.5) -> PyTree:
    ...
    return jax.tree_util.tree_map(lambda gi, gc: (1 - beta) * gi + beta * gc, g_ind, g_col)
...
def compute_fcgrad(
    ...
    grads = jax.tree_util.tree_map(lambda a, b, c: a + b + c, g_actor, g_critic_ind, g_critic_col)
    loss_info = {
